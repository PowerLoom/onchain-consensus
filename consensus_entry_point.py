...
from data_models import Epoch
from data_models import EpochConsensusStatus
from data_models import EpochData
from data_models import EpochDataPage
from data_models import EpochDetails
from data_models import EpochInfo
from data_models import EpochStatus
from data_models import EpochStatusRequest
from data_models import Message
from data_models import PeerRegistrationRequest
from data_models import ProjectSpecificSnapshotters
from data_models import SnapshotBase
...
from data_models import SubmissionResponse
from data_models import SubmissionStatus
from helpers.redis_keys import *
from helpers.state import check_consensus
from helpers.state import get_project_finalized_epoch_cids
from helpers.state import get_submission_schedule
from helpers.state import prune_finalized_cids_htable
from helpers.state import register_submission
from helpers.state import submission_delayed
    ...
    while True:
        ...
        async for project_id in app.state.reader_redis_pool.scan_iter(match=project_id_pattern):
            ...
            pruning_tasks.append(
                prune_finalized_cids_htable(
                    project_id, app.state.reader_redis_pool,
                ),
            )
...
async def startup_boilerplate():
    ...
    app.state.w3 = Web3(
        Web3.HTTPProvider(
            settings.anchor_chain_rpc.full_nodes[0].url,
        ),
    )
...
async def register_peer_against_project(
    ...
    try:
        ...
        service_logger.info(
            'Peer Registration transaction sent! Transaction hash: {}', tx_hash,
        )
...
async def submit_snapshot(
    ...
    try:
        ...
        logger.error(
            'Snapshot submission transaction failed with error: {}', E,
        )
    ...
    if await submission_delayed(
        ...
        response_obj = SubmissionResponse(
            status=SubmissionAcceptanceStatus.accepted, delayedSubmission=True,
        )
    else:
        response_obj = SubmissionResponse(
            status=SubmissionAcceptanceStatus.accepted, delayedSubmission=False,
        )
...
async def epoch_details(
    ...
    epoch_release_time = await app.state.reader_redis_pool.zscore(
        ...
        json.dumps(
            {'begin': epoch - settings.chain.epoch.height + 1, 'end': epoch},
        ),
    )
...
async def epoch_status(
    ...
    for idx, result in enumerate(results):
        ...
        return_map[epoch_query_list[idx]] = SubmissionResponse(
            status=status, delayedSubmission=False, finalizedSnapshotCID=finalized_cid,
        ).dict()
    return return_map
...
@app.get(
    '/metrics/{project_id}/snapshotters', response_model=ProjectSpecificSnapshotters,
...
async def get_epochs(
    ...
    epoch_ends = sorted(
        list(
            set(
                [int(key.decode('utf-8').split(':')[2]) for key in epoch_keys],
            ),
        ), reverse=True,
    )
    ...
    epochs = []
    epoch_status_tasks = [
        bound_check_epoch_finalization(
            project_id, epoch_end, request.app.state.reader_redis_pool, semaphore=semaphore, request=request,
        )
    ...
    for i in range(len(epoch_ends_data)):
        ...
        epochs.append(
            Epoch(
                sourcechainEndheight=epoch_ends_data[i], finalized=finalized,
            ),
        )
    ...
    return {
        ...
        'next_page': None if page * limit >= len(
            epoch_ends,
        ) else f'/metrics/{project_id}/epochs?page={page + 1}&limit={limit}',
...
async def get_submission_status(
    ...
    if not submission_data:
        return JSONResponse(
            status_code=404,
            content={
                'message': f'Project with projectID {project_id} and epoch {epoch} not found',
            },
        )
...
async def get_current_epoch(
    ...
    if epoch_end_block_height is None:
        return JSONResponse(
            status_code=404,
            content={
                'message': 'Epoch not found! Make sure the system ticker is running.',
            },
        )
