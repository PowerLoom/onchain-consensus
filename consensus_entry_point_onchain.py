...
async def epoch_details(
    ...
    if not (
        ...
        return inject_rate_limit_fail_response(rate_limit_auth_dep)
    if epoch == 0:
        epoch_begin, epoch_end = (await request.app.state.rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.currentEpoch()],
            app.state.writer_redis_pool
        ))[0]
        if epoch_end == 0:
            return JSONResponse(status_code=404, content={"message": f"No epoch found with given Epoch End Time"})
    
    epoch_release_time = (await request.app.state.rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.epochReleaseTime(epoch)],
            app.state.writer_redis_pool
        ))[0]
    if epoch_release_time == 0:
        return JSONResponse(status_code=404, content={"message": f"No epoch found with given Epoch End Time"})
    
    projects = (await request.app.state.rpc_helper.web3_call(
        [request.app.state.protocol_state_contract.functions.getProjects()],
        app.state.writer_redis_pool
    ))[0]

    project_epoch_status_tasks = []

    for project_key in projects:
        project_epoch_status_tasks.append(request.app.state.protocol_state_contract.functions.epochStatus(project_key, epoch))
    
    project_epoch_status_results = await request.app.state.rpc_helper.web3_call(
        project_epoch_status_tasks,
        app.state.writer_redis_pool
    )

    finalized_projects_count = 0
    for project_key, project_epoch_status_result in zip(projects, project_epoch_status_results):
        if project_epoch_status_result:
...
async def epoch_status(
    ...
    epoch_status = (await request.app.state.rpc_helper.web3_call(
        [request.app.state.protocol_state_contract.functions.epochStatus(req_parsed.projectID, req_parsed.epoch.end)],
        app.state.writer_redis_pool
    ))[0]
    ...
    if epoch_status:
        finalized_cid = (await request.app.state.rpc_helper.web3_call(
            [request.app.state.protocol_state_contract.functions.maxSnapshotsCid(req_parsed.projectID, req_parsed.epoch.end)],
            app.state.writer_redis_pool
        ))[0]  
...
async def get_snapshotters(
    ...
    """
    snapshotters = (await request.app.state.rpc_helper.web3_call(
        [request.app.state.protocol_state_contract.functions.getSnapshotters(project_id)],
        app.state.writer_redis_pool
    ))[0]
...
async def get_current_epoch(
    ...
    return {
        ...
        "epochStartBlockHeight": epoch_end,
        "epochEndBlockHeight": epoch_begin
    }
